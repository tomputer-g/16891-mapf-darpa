...
Drone agents are tracked separately and excluded from CBS collision resolution, since they operate at a different altitude than ground agents. Drones are planned independently using Manhattan-heuristic A* with no inter-agent constraints.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{darpa3} ($16 \times 15$): Same as \texttt{darpa1} but with an obstacle-filled bottom row; agents start in a 5-cell gap along this row, forcing coordinated egress.
        \item \texttt{darpa4} ($16 \times 15$): Same treatment applied to \texttt{darpa2}---obstacle spawn row with constrained start positions.
        \item \texttt{darpa5} ($16 \times 15$): Maze configuration with single-width corridors and the obstacle spawn row. Tests narrow-passage coordination.
        \item \texttt{darpa6} ($22 \times 21$): Larger maze with 2-block-wide pathways, 4 objectives, 4 buildings. Tests scalability on bigger maps.
        \item \texttt{darpa7} ($22 \times 21$): Large open layout (no maze) with 8 objectives and 6 buildings. Tests high task density.
    \end{itemize}
\end{itemize}
...
\begin{enumerate}
    ...
    \item Zero reauctions were triggered in all SSIA runs, indicating that CBS-based collision avoidance and reward-shaped allocation produce stable initial assignments.
\end{enumerate}
...
\textbf{Joshua Pen}: SSIA task allocation with reward-shaped bidding, agent model, SSIA simulation harness, scenario design, triage task integration.
