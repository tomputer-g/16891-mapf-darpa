...
class SequentialSingleItemAuctioneer(BaseSSIATaskAuctioneer):
    ...
    # ------------------------------------------------------------------
    # Exploration variant: seed exploration tasks for the whole map
    # ------------------------------------------------------------------
    def add_global_exploration_tasks(self, known_map: KnownMap) -> int:
        """Register one ExplorationTask for every currently UNKNOWN cell.

        This exhaustive exploration variant assumes the agents know the map bounds.
        Deduplication is handled by ``register`` via ``_known_locs`` so cells
        already promoted to tasks are skipped on subsequent ticks.
        """
        new_count = 0
        for r in range(known_map.rows):
            for c in range(known_map.cols):
                if known_map.state[r][c] != ObservationState.UNKNOWN:
                    continue
                if self.register(ExplorationTask((r, c))):
                    new_count += 1
        return new_count
    ...
    def sweep_completions(self, known_map: KnownMap) -> int:
        ...
        if count:
            ...
            self._purge_collateral_exploration_tasks()
        return count

    def _purge_collateral_exploration_tasks(self) -> None:
        """Drop completed ExplorationTasks from the master task list.

        Exploration tasks complete the moment their target cell becomes
        non-UNKNOWN, so on this exhaustive variant they accumulate fast and
        bloat ``self._tasks`` (and the stats counts) without contributing to
        the auction. This removes them once they're swept, keeping any task
        that is still an agent's *current* head task — the main loop will
        clear that reference on the next tick when it advances the queue.
        """
        active_ids = {
            id(t)
        ...
        for t in self._tasks:
            if (isinstance(t, ExplorationTask)
                    and t.completed
                    and id(t) not in active_ids):
                continue
            kept.append(t)
        self._tasks = kept
    ...
    def all_complete(self) -> bool:
        """Empty list now means 'everything was swept'; treat as complete."""
    ...
    def auction(self, agents: List[Agent], known_map: KnownMap) -> None:
        ...
        skipped_collateral = 0

        for task in available_tasks:
            # Skip exploration tasks whose target cell is already inside
            # an existing or freshly-won queued path's observation footprint.
            # These cells will be swept on observation, so auctioning them
            # just adds queue noise. Triage / investigation tasks always
            # proceed (they require dwelling, not just observation).
            if (isinstance(task, ExplorationTask)
                    and task.target_loc in self._predicted_revealed):
        ...
        if skipped_collateral:
            print(f"  [AUCTION] Skipped {skipped_collateral} exploration "
                  "task(s) covered by queued path footprints")
    ...
    def update(self, agents: List[Agent], known_map: KnownMap,
        ...
        new_explore = self.add_global_exploration_tasks(known_map)
        if new_explore and verbose:
            print(f"  [EXPLORE] +{new_explore} exploration task(s) queued")
