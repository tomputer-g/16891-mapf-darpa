...
Uses the Availability-Constrained Sequential Single-Item Auction from
``SSIA.task_allocation`` on a scenario file, defaulting to
...
class SSIAHarness(SimulationHarness):
    name = "SSIA Exploration Simulation"
...
def run_simulation(
    ...
    return SSIAHarness().run_simulation(path, max_steps, verbose, use_vis)
